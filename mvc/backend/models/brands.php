...
class brands extends model {
	...
	public function update($request) {
		$con = $this->connection();
		$name = ucfirst($request['name']);
		$queryUpdate = "UPDATE brands SET name = '$name' , status = '{$request['status']}', created_by = '{$request['created_by']}' where ID = '{$request['id']}'";
	...
	public function getByCatelogyId($id) {
		$con = $this->connection();
		$querySelect = "SELECT * FROM brands WHERE category_id = $id";
		// die($id);
		$result = mysqli_query($con, $querySelect);
		$brandsByCategoryId = [];
		if (mysqli_num_rows($result) > 0) {
			$brandsByCategoryId = mysqli_fetch_all($result, MYSQLI_ASSOC);
		}
		return $brandsByCategoryId;
	}
