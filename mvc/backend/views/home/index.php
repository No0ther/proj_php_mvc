...
$datetime = date("Y-m-d H:i:s", time()); // Hàm lấy thời gian thực
$current = strtotime($datetime); // Đổi thời gian hiện sang dạng int
$time = strtotime($noti['created_at']); // Đổi thời gian tạo tbao sang int
$interval = $current - $time; // 10h36 - 10h20 => 16p trước => 16*60 = 960s
if ($interval > 0 && $interval < 60) {
	//54s
	$time_text = $interval . 's';
} elseif ($interval >= 60 && $interval < 60 * 60) {
	// quá 1m
	$time_text = round($interval / 60, 0) . 'm';
} elseif ($interval >= 60 * 60 && $interval < 60 * 60 * 24) {
	// quá 1h
	$time_text = round($interval / 3600, 0) . 'h';
} elseif ($interval >= 60 * 60 * 24 && $interval < 60 * 60 * 24 * 30) {
	// quá 1 ngày
	$time_text = round($interval / 86400, 0) . 'd';
} elseif ($interval >= 60 * 60 * 24 * 30 && $interval < 60 * 60 * 24 * 30 * 12) {
	// quá 1 tháng
