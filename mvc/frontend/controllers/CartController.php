...
class CartController extends Controller {
	...
	public function purchase() {
		...
		if (isset($_POST['purchase'])) {
			...
			$billModel = new bill();
			$result = $billModel->insert($request); // Tạo hóa đơn và chi tiết hóa đơn

			$updateStockModel = new bill();
			$result2 = $updateStockModel->updateStock($request); // Cập nhật lại kho sau khi Người dùng mua hàng

			$mailBillModel = new bill();
			$resultMail = $mailBillModel->mail($request); // Gửi mail confirm cho khách hàng

			$noti = new notifications(); // Tạo thông báo
